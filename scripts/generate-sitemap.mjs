...
// Falls back to the confirmed production domain if VITE_SITE_URL isn't
// set — see IMPLEMENTATION_STATE.md (Package 6.4) for why this isn't
// hardcoded-only: once a staging/preview env sets VITE_SITE_URL, this
// script (and src/lib/seo.js's getSiteUrl(), the same precedent)
// picks it up automatically.
const SITE_URL = (
  env.VITE_SITE_URL || "https://www.urbanedgelivingspace.com"
).replace(/\/+$/, "");
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.warn(
      "[generate-sitemap] VITE_SUPABASE_URL/VITE_SUPABASE_ANON_KEY not set — " +
        "writing a static-routes-only sitemap.xml. Set both env vars and re-run " +
        "`npm run build` to include live property/blog slugs.",
    );
  } else {
