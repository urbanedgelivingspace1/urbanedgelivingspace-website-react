...
const PropertyModule = () => {
  const initialState = {
    ...
    project_area: '',
    ownership: '',
    floor: '',             // changed from towers_floor
    towers: '',            // new field added
    price: '',
    property_view: '',
    parking: '',
    about_property: '',
  ...
  const handleChange = (e) => {
    const updated = { ...propertyData, [e.target.name]: e.target.value };
  ...
  const handleEditImageChange = (e) => setEditingImageFile(e.target.files[0]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...propertyData,
        image_url,
        amenities: JSON.stringify(propertyData.amenities.split(',').map(a => a.trim())),
        floor_space_pricing: JSON.stringify(propertyData.floor_space_pricing.split(',').map(a => a.trim()))
      };
  ...
  const startEdit = (property) => {
    ...
    setEditingData({
      ...
      floor_space_pricing: Array.isArray(property.floor_space_pricing) ? property.floor_space_pricing.join(', ') : property.floor_space_pricing
    });
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...editingData,
        image_url,
        amenities: JSON.stringify(editingData.amenities.split(',').map(a => a.trim())),
        floor_space_pricing: JSON.stringify(editingData.floor_space_pricing.split(',').map(a => a.trim()))
      };
          ...
          properties.map((property) => (
            <div key={property.id} className="property-card">
              ...
              <div className="property-details">
                <h3>{property.name}</h3>
                <p className="property-location"><FaMapMarkerAlt className="icon" /> {property.location}</p>
                <p className="property-bhk"><FaBed className="icon" /> {property.bhk} BHK</p>
                <p className="property-sq"><FaRuler className="icon" /> {property.carpet_area} Sq Ft</p>
              </div>
              <div className="property-actions">
                <button onClick={() => startEdit(property)} className="btn btn-edit">Edit</button>
                <button onClick={() => deleteProperty(property.id)} className="btn btn-delete">Delete</button>
              </div>
            </div>
          ))
        )}
          ...
          <div className="modal-card">
            ...
            <form onSubmit={handleEditSubmit}>
              {Object.entries(initialState).map(([key]) => {
                ...
                return (
                  <div key={key} className="form-group">
                    <label htmlFor={`edit-${key}`}>{key.replace(/_/g, ' ').toUpperCase()}</label>
                    <input
                      type={key === 'possession' ? 'date' : 'text'}
                      id={`edit-${key}`}
                      name={key}
              ...
              <div className="modal-actions">
                ...
                <button type="button" onClick={cancelEdit} className="btn btn-cancel">Cancel</button>
              </div>
            </form>
          </div>
        </div>
