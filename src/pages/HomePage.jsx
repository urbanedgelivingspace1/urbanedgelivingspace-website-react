...
const HeroSection = () => {
  ...
  // Live count of published listings — replaces the old hardcoded "50+".
  // usePropertiesCount runs a `head: true` Supabase query, so it only ever
  // transfers the count, never the rows, however many properties there
  // are. No filters are passed, so this matches every published property
  // regardless of buy/rent/commercial type — the same total shown on the
  // Properties page with no filters applied.
  const {
    data: statsData,
    isLoading: loadingPropertyCount,
    isError: propertyCountFailed,
  } = usePropertiesCount();

  const totalPropertiesCount = statsData?.count ?? null;
  const hasLivePropertyCount = !loadingPropertyCount && !propertyCountFailed
        ...
        <div className="homepage-hero-stats">
          <div className="homepage-hero-stat">
            ...
            <span className="homepage-hero-stat-value">
              {hasLivePropertyCount ? (
                `${totalPropertiesCount}+`
              ) : propertyCountFailed ? (
                // Genuine fetch failure (e.g. offline) — degrade to
                // wording that doesn't claim a specific, possibly wrong,
                // number rather than silently showing a fake count.
                "New"
              ) : (
                <Skeleton
                  variant="text"
                  width={40}
                  height="1em"
                  className="homepage-hero-stat-skeleton"
                />
              )}
            </span>
